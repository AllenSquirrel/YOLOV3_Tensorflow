...
with tf.variable_scope('yolov3'):
    ...
    y_pred = model.predict(pred_feature_map)# 计算预测图 最终输出结果以y向量形式表示

tf.summary.scalar("loss/coord_loss", loss[1])#中心坐标
tf.summary.scalar("loss/sizes_loss", loss[2])#box尺寸  w，h
tf.summary.scalar("loss/confs_loss", loss[3])#box是否包含有object
tf.summary.scalar("loss/class_loss", loss[4])#类别预测误差
...
# print(a1 == a2)
# exit()
# 恢复darknet-53特征提取器的权重参数, 只更新yolo-v3目标预测部分参数.即迁移学习
...
saver_to_restore.restore(sess, "../data/checkpoint/yolov3.ckpt")#tf保存模型 以.ckpt为后缀  其位训练得到的模型结构
