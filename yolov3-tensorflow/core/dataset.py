...
class Parser(object):
    ...
    def preprocess_true_boxes(self, gt_boxes):
        ...
        y_true: list(3 array), shape like yolo_outputs, [13, 13, 3, 85]
                            ...
                            85: box_centers, box_sizes, confidence, probability
                              即中心坐标,长宽,是否存在目标的概率,属于哪一类class的概率
        """
        ...
        # 即[[6, 7, 8], [3, 4, 5], [0, 1, 2]]； 
        # 如果只有2个feature_map 则取[3, 4, 5], [0, 1, 2]
        ...
        wh = np.expand_dims(wh, -2)  # 使得能够进行np.maximum(boxes_min, anchors_min)广播
        boxes_max = wh / 2.
        boxes_min = -boxes_max
    ...
    def parser_example(self, serialized_example):
        ...
        gt_boxes = tf.reshape(gt_boxes, shape=[-1, 5])  # 5维 坐标, 长度, 属于3个anchor中的哪一个anchor（目标中心点所属问题）

        return self.preprocess(image, gt_boxes)

#此部分为执行训练前预处理操作  （1）数据增强 （2）通过yolov3核心算法将boxes处理成网络训练的格式 
# 不同尺度下进行的标签的处理 cell x cell x anchors_number x (5(box_centers, box_sizes, confidence) + class_number)  
# 即为S*S*(B*5+C)
class dataset(object):
    ...
    def get_next(self):#迭代器执行迭代操作 不断获取数据集放入网络进行训练  
