...
class darknet53(object):
    """用于执行特征提取的网络  此部分可以作为预训练模型 提取较为浅层的普适特征
...
class yolov3(object):
    ...
    def _detection_layer(self, inputs, anchors):
        ...
        return feature_map
    #得到三种不同尺度下的feature map 每一feature map以向量形式输出 包括预测目标框，是否有目标 ，分类概率
    ...
    def _reorg_layer(self, feature_map, anchors):
        ...
        a, b = tf.meshgrid(grid_x, grid_y)  # 构建网格 
        '''
        a=[0,5,10]
        b=[0,5,15,20,25]
        A,B=tf.meshgrid(a,b)
    ...
    def forward(self, inputs, is_training=False, reuse=False):
        ...
        :return:
        """
        # it will be needed later on 在稍后将被需要
        self.img_size = tf.shape(inputs)[1:3]
        # set batch norm params设定归一化层参数
        batch_norm_params = {
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm, common._fixed_padding], reuse=reuse):
            with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                ...
                with tf.variable_scope('yolo-v3'):
                    ...
                    feature_map_1 = tf.identity(feature_map_1, name='feature_map_1')#13*13
                    ...
                    feature_map_2 = tf.identity(feature_map_2, name='feature_map_2')#26*26
                    ...
                    feature_map_3 = tf.identity(feature_map_3, name='feature_map_3')#52*52
    ...
    def compute_loss(self, pred_feature_map, y_true, ignore_thresh=0.5, max_box_per_image=8):
        ...
        total_loss = loss_xy + loss_wh + loss_conf + loss_class#可参照yolov3 损失函数公式
