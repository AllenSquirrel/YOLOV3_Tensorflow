...
#非最大值抑制算法（间接调用IOU计算）   此部分将会严重影响最终的预测框的精度  
#非最大值抑制算法以IOU值作为先行判断条件

# Discard all boxes with low scores and high IOU 丢弃所有低分和高IOU的box,
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.3, iou_thresh=0.5):
    ...
    for i in range(num_classes):
        ...
        boxes_list.append(tf.gather(filter_boxes, nms_indices))  # 第几个c(c∈10647)中含有第i类box(4维张量->box坐标尺寸)
...
#根据训练按照batch进行训练 最终会对每一类的预测结果进行平均计算
