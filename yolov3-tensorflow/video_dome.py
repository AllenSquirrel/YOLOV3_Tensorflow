...
graph = tf.Graph()
#根据配置makefile文件中硬件环境配置不同 ，采用不同加速模式 CPU  GPU
if flags.model == "cpu":
...
with tf.Session(graph=graph) as sess:
    vid = cv2.VideoCapture(flags.video_id)#通过第三方库opencv驱动摄像头
    while True:
        return_value, frame = vid.read()
        if return_value:
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)#frame由opencv默认BGR转换为RGB 方便后续操作（可读性增强）
            image = Image.fromarray(frame)
        else:
            raise ValueError("No image!")
        img_resized = np.array(image.resize(size=(IMAGE_H, IMAGE_W)), dtype=np.float32)#图像大小  设定尺寸以及浮点数类型（注意在resize大小转换过程中，必须将图片格式转换为浮点数  ，由opencv默认规定）
        ...
        image = utils.draw_Chinese(image, "按k截图", (0, 55))#人机交互  可直接观测预测效果及结果并截图保存
        curr_time = time.time()
        exec_time = curr_time - prev_time
        result = np.asarray(image)
        info = "time: %.2f ms" % (1000 * exec_time)#给出每一张图片 或每一帧图片的预测时间
